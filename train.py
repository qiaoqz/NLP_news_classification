...
# Please change the data_dir to the directory where you plan to save the checkpoints of every 100 steps.
tf.flags.DEFINE_string("data_dir", "/Users/QiaoQiao/Desktop/Changed_TopicCla_News", "Provide directory location")
...
print("Loading data...")
# Load training
word_index_pickle = open('word_index_pickle_train', 'rb')
pickling = pickle.load(word_index_pickle)
x_train = pickling['word_indices']
y_train = pickling['y']
...
# Load valid
word_index_pickle = open('word_index_pickle_valid', 'rb')
pickling = pickle.load(word_index_pickle)
x_dev= pickling['word_indices']
y_dev = pickling['y']
word_index_pickle.close()
######
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        optimizer = tf.train.AdamOptimizer(1e-3) # GradientDescentOptimizer(learning_rate = 0.001)
	...
	# Assigning glove vector to the embedding vector	
        g = open('glove.6B.100d_pickle', 'rb')
        ...
        # Generate batches
        batches = clean_helper.batch_iter(
        ...
        # Save the model.
        # Please change the directory(path before 'my_model.ckpt') to your folder name.
        saver.save(sess,'./my_model_probability/my_model.ckpt')
